...
| Playtest note export | `client/src/results/ResultsOverlay.ts`, `client/src/ui/lobbyView.ts`, `client/vite.config.ts`, `scripts/validate-results-overlay.ts`, `scripts/validate-settlement-details.ts` | Results overlay and lobby recent-run summary both expose a structured copyable manual playtest note from settlement data, including build commit, pressure phase, combat contacts, item value, loadout loss, and timestamp prompts; the results card also shows the build tag directly so testers can capture the required evidence quickly |
...
- Human playtest note capture: the note-copy action now exists in both the results overlay and lobby recent-run summary, and it now pre-fills build commit, structured pressure, combat, item-value, loadout-loss, and timestamp evidence; the results overlay now also shows the build tag directly. The session itself still needs to be run and rated.
