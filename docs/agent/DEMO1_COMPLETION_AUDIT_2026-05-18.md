...
| Item payoff presentation | `shared/src/data/items.ts`, `client/src/ui/itemPresentation.ts`, `client/src/results/ResultsOverlay.ts`, `client/src/ui/lobbyView.ts`, `client/src/ui/marketView.ts`, `scripts/validate-item-presentation.ts` | Covered for every shared item definition: no raw id names, no generic misc fallback, bitmap icon assets exist, definition-id translation works, and settlement/lobby/black-market payoff rows use curated presentation names |
| Backpack grid and equipment slots | `shared/src/domain/inventory.ts`, `client/src/ui/InventoryPanel.ts`, `validate:profile-carry`, `validate:drag-contracts` | Covered; drag and swap semantics for combat inventory, stash, and equipment are now inside the GDD release gate |
...
| Release-level verification gate | `package.json` `validate:gdd-demo1-contract` and `validate:release-readiness` | Passed on 2026-05-18 at `a6e8421`; includes multiclient, combat, loot, tactical consumables, item payoff presentation, carry, extract, audio, market, pressure, monster tuning, settlement replay prompt behavior, drag/swap contracts, mobile action controls, typecheck, build, and late-game smoke |
| Browser visual acceptance | `package.json` `accept:visual-readiness`, `.codex-artifacts/game-feel-baseline/` screenshots | Passed on 2026-05-18 at `a6e8421`; covers boss combat HUD, inventory overlay, and late-game extraction pressure visibility; not a manual fun signoff |
...
| Manual release-feel protocol | `docs/agent/MANUAL_PLAYTEST_PROTOCOL_2026-05-18.md`, `package.json` `playtest:manual`, `scripts/dev.mjs`, `validate:manual-playtest-entrypoint` | Defines the required 9-12 minute human playtest scorecard, pass/fail rules, and output format; the manual entrypoint prints the URL/protocol and auto-stops after 20 minutes for resource hygiene, and the entrypoint contract is guarded in the GDD gate; still requires execution |
...
- Manual long-session balance: 9-12 minute ideal extraction timing cannot be proven by deterministic tests; `MANUAL_PLAYTEST_PROTOCOL_2026-05-18.md` now defines the required evidence format, `npm run playtest:manual` provides the resource-managed entrypoint, and `validate:manual-playtest-entrypoint` guards that workflow from regressing.
...
- `npm run validate:launch-readiness` passed at `a6e8421` and is tagged as `launch-readiness-checkpoint-20260518-k`.
...
- `validate:manual-playtest-entrypoint` now guards the resource-managed manual playtest launcher, protocol reference, 20-minute auto-stop, and completion-audit handoff text.
...
- `validate:item-presentation` now guards every shared item definition against raw-id display names, generic misc fallback, missing bitmap icons, and broken definition-id translation; `validate:results-overlay`, `validate:settlement-details`, and `validate:market-lifecycle` also guard settlement cards, lobby recent-run chips, and black-market rows against raw item-name display.
