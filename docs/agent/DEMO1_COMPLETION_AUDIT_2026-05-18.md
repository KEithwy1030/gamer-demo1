...
| Manual release-feel protocol | `docs/agent/MANUAL_PLAYTEST_PROTOCOL_2026-05-18.md`, `package.json` `playtest:manual`, `scripts/dev.mjs` | Defines the required 9-12 minute human playtest scorecard, pass/fail rules, and output format; the manual entrypoint prints the URL/protocol and auto-stops after 20 minutes for resource hygiene; still requires execution |
...
- Manual long-session balance: 9-12 minute ideal extraction timing cannot be proven by deterministic tests; `MANUAL_PLAYTEST_PROTOCOL_2026-05-18.md` now defines the required evidence format and `npm run playtest:manual` provides the resource-managed entrypoint.
