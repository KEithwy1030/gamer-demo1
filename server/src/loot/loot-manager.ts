...
import type { Affix, AffixKey, EquipmentSlot as SharedEquipmentSlot, ItemRarity } from "../../../shared/dist/types/inventory.js";
...
export function createDropsForMonster(room: RuntimeRoom, monster: RuntimeMonster): DropState[] {
  ...
  definitionIds.forEach((definitionId, index) => {
    const item = buildInventoryItem(definitionId);
    if (!item) {
      return;
    }
...
function buildInventoryItem(definitionId: string): InventoryItem | undefined {
  ...
  return {
    ...
    modifiers: definition.stats
      ...
      : undefined,
    affixes: rollAffixes(equipmentSlot, definition.rarity)
  };
}

function toInventoryKind(category: "weapon" | "armor" | "gold" | "treasure"): InventoryItem["kind"] {
  if (category === "weapon") {
    return "weapon";
  }
