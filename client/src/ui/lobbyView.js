...
const renderPlayers = (roomState, playerList) => {
    ...
    roomState.players.forEach((player, index) => {
        ...
        if (player.id === roomState.localPlayerId) {
            item.append(createElement("span", "player-card__badge", "YOU"));
        }
...
export class LobbyView {
    ...
    constructor(controller, runtimeApi, callbacks) {
        ...
        const heroEyebrow = createElement("p", "hero-panel__eyebrow", "Demo 1 / Extraction Game");
        const heroTitle = createElement("h1", "hero-panel__title", "搜打撤");
        const heroBody = createElement("p", "hero-panel__body", "末日废土中的战利品争夺战。组建你的小队，进入危险区域，搜刮物资并活着撤离。");
        const heroTags = createElement("div", "hero-panel__tags");
        ["PIXEL ART", "MULTIPLAYER", "HARDCORE"].forEach((text) => {
        ...
        actionCard.append(createElement("p", "panel__eyebrow", "LOGIN"), createElement("h2", "panel__title", "准备就绪"));
        const form = createElement("div", "input-stack");
        const nameLabel = createElement("label", "field");
        nameLabel.append(createElement("span", "field__label", "代号 / AGENT NAME"));
        ...
        roomLabel.append(createElement("span", "field__label", "频道 / CHANNEL CODE"));
        ...
        roomHeadingText.append(createElement("p", "panel__eyebrow", "COMMUNICATION"), createElement("h2", "panel__title", "小队集结"));
        this.phasePill = createElement("span", "status-pill", "WAITING...");
        roomHeading.append(roomHeadingText, this.phasePill);
        const stats = createElement("div", "room-stats");
        this.roomCodeValue = createInfoRow("频道号", "------");
        ...
        playersWrap.append(createElement("div", "section-heading", "ROSTER"));
        ...
        [2, 3, 4].forEach((value) => {
            ...
            option.textContent = `${value} AGENTS`;
        ...
        this.leaveButton = createElement("button", "button button--ghost", "断开连接");
    ...
    render(state) {
        ...
        if (!roomState) {
            ...
            this.playerList.replaceChildren(createElement("li", "player-list__placeholder", "Waiting for team authorization..."));
        ...
        this.phasePill.textContent = roomState.status === "starting" ? "DEPLOYING..." : "READY";
