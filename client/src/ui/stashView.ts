import type { EquipmentSlot, PlayerProfilePayload } from "@gamer/shared";

type LoadoutItem = PlayerProfilePayload["inventory"]["items"][number];
type EquipmentItem = NonNullable<PlayerProfilePayload["equipment"][EquipmentSlot]>;
type PendingItem = NonNullable<PlayerProfilePayload["pendingReturn"]>["items"][number];
type StashItem = PlayerProfilePayload["stash"]["pages"][number]["items"][number];
type RenderableItem = LoadoutItem | EquipmentItem | PendingItem | StashItem;

type SelectedItemRef =
  | { area: "loadout-grid"; item: LoadoutItem }
  | { area: "loadout-equipment"; item: EquipmentItem; slot: EquipmentSlot }
  | { area: "stash"; item: StashItem; pageIndex: number }
  | { area: "pending"; item: PendingItem };

type CategoryFilter = "all" | "weapon" | "head" | "chest" | "hands" | "shoes" | "treasure" | "consumable" | "material" | "quest";
...
export interface StashViewCallbacks {
  ...
  onEquipItem(itemInstanceId: string): void;
...
export class StashView {
  ...
  private profile: PlayerProfilePayload | null = null;
  ...
  constructor(callbacks: StashViewCallbacks) {
    ...
    headerLeft.append(
      ...
      this.makeTitle("行囊"),
      this.makeSub("营地库房 · 共 19 件 · 承重整理中")
    );
    ...
    detail.append(this.makeStamp("详情 · WEAPON"));
    ...
    this.detailEmpty.textContent = "点击任意物品后，在这里执行入库、装备、转移或丢弃。";
  ...
  render(profile: PlayerProfilePayload | null): void {
    ...
    this.goldValue.textContent = formatCompactNumber(profile?.stashGold ?? 0);
    this.renderCapacity();
  ...
  private renderPendingBanner(): void {
    ...
    this.pendingBanner.textContent = pendingCount > 0
      ? `本次撤离有 ${pendingCount} 件物资暂未入库，必须先处理后才能继续下一局。`
      : "";
  }
  ...
  private renderLoadout(): void {
    ...
    for (const slot of EQUIPMENT_ORDER) {
      ...
      if (item) {
        ...
        if (this.selected?.area === "loadout-equipment" && this.selected.item.instanceId === item.instanceId) {
        ...
        card.addEventListener("click", () => {
          this.selected = { area: "loadout-equipment", item, slot };
          this.render(this.profile);
        });
        slotEl.append(card);
      } else {
    ...
    (loadout?.items ?? []).forEach((item) => {
      this.loadoutGrid.append(
        this.buildGridItem({ area: "loadout-grid", item }, item.x, item.y, item.width ?? 1, item.height ?? 1)
      );
    });
  }
  ...
  private resolveSelected(selected: SelectedItemRef): SelectedItemRef | null {
    ...
    switch (selected.area) {
      ...
      case "loadout-grid": {
        ...
        return item ? { area: "loadout-grid", item } : null;
      }
      case "loadout-equipment": {
        const item = profile.equipment[selected.slot];
        return item?.instanceId === selected.item.instanceId ? { area: "loadout-equipment", item, slot: selected.slot } : null;
      }
      case "stash": {
...
function buildActionsForSelected(selected: SelectedItemRef, currentPageIndex: number): ActionItem[] {
  ...
  const equipSlot = selected.item.equipmentSlot as EquipmentSlot | undefined;
  if (selected.area === "pending") {
    return [
      { label: "装备", payload: { itemInstanceId: itemId, targetArea: equipSlot ? "equipment" : "grid", slot: equipSlot } },
      { label: "存入当前页", payload: { itemInstanceId: itemId, targetArea: "stash", pageIndex: currentPageIndex } },
      { label: "丢弃", payload: { itemInstanceId: itemId, targetArea: "discard" } }
    ];
  }
  if (selected.area === "stash") {
    const actions: ActionItem[] = [{ label: "转入携行", payload: { itemInstanceId: itemId, targetArea: "grid" } }];
    if (equipSlot) {
      actions.unshift({ label: "装备", payload: { itemInstanceId: itemId, targetArea: "equipment", slot: equipSlot } });
    }
    actions.push({ label: "丢弃", payload: { itemInstanceId: itemId, targetArea: "discard" } });
    return actions;
  }
  if (selected.area === "loadout-grid") {
  ...
  return [
    { label: "卸到背包", payload: { itemInstanceId: itemId, targetArea: "grid" } },
    { label: "存入仓库", payload: { itemInstanceId: itemId, targetArea: "stash", pageIndex: currentPageIndex } }
  ];
}

function buildCompactMeta(item: RenderableItem): string {
...
function buildDescription(item: RenderableItem): string {
  const parts: string[] = [];
  if (typeof item.goldValue === "number" && item.goldValue > 0) {
    parts.push(`基础价值 ${formatCompactNumber(item.goldValue)}`);
  }
  if (typeof item.treasureValue === "number" && item.treasureValue > 0) {
    parts.push(`上局收益 ${formatCompactNumber(item.treasureValue)}`);
  }
  if (typeof item.healAmount === "number" && item.healAmount > 0) {
    parts.push(`恢复 ${item.healAmount} HP`);
  }
  return parts.join(" · ") || "暂无额外说明。";
}

function buildStatRows(item: RenderableItem): Array<{ label: string; value: string }> {
  const rows = [{ label: "尺寸", value: `${item.width ?? 1}×${item.height ?? 1}` }];
  if (item.equipmentSlot) {
    rows.push({ label: "品级", value: EQUIPMENT_LABELS[item.equipmentSlot] });
  }
  if (item.weaponType) {
    rows.push({ label: "类型", value: item.weaponType });
  }
  return rows;
}

function matchesCategory(item: RenderableItem, category: CategoryFilter): boolean {
  if (category === "all") {
    return true;
  }
  if (category === "weapon") {
    return item.equipmentSlot === "weapon" || item.kind === "weapon";
  }
  if (category === "head" || category === "chest" || category === "hands" || category === "shoes") {
    return item.equipmentSlot === category;
  }
  if (category === "consumable") {
    return item.kind === "consumable" || typeof item.healAmount === "number";
  }
  if (category === "treasure") {
    return item.kind === "treasure" || typeof item.treasureValue === "number";
  }
  if (category === "material") {
    return item.kind !== "weapon" && item.equipmentSlot == null && item.treasureValue == null && item.healAmount == null;
  }
  if (category === "quest") {
    return false;
  }
...
function rarityLabel(rarity: RenderableItem["rarity"]): string {
  switch (rarity) {
    case "uncommon":
      return "A";
    case "rare":
