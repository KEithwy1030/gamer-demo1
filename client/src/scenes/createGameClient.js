...
export function createGameClientController(options) {
    ...
    const controller = {
        ...
        setCombatResult(payload) {
            runtime.setCombatText(`战斗: ${payload.attackerId.slice(0, 4)} -> ${payload.targetId.slice(0, 4)} -${payload.amount}`);
    ...
    subscriptions.push(network.onMatchStarted((payload) => controller.enterMatch(payload)), network.onPlayersState((players) => controller.applyPlayers(players)), network.onMonstersState((monsters) => controller.applyMonsters(monsters)), network.onDropsState((drops) => controller.applyDrops(drops)), network.onInventoryUpdate((payload) => controller.setInventory(payload)), network.onCombatResult((payload) => controller.setCombatResult(payload)), network.onMatchTimer((secondsRemaining) => controller.setTimer(secondsRemaining)), network.onExtractOpened((payload) => {
        ...
        const selfPlayerId = controller.getSelfPlayerId();
        if (payload?.playerId && payload.playerId !== selfPlayerId)
            return;
        controller.setExtractState({
            isOpen: true,
            isExtracting: false,
    ...
    function mount() {
        ...
        game.scene.start(GameScene.KEY, {
            runtime,
            extractState,
            onMoveInput: (direction) => network.sendMoveInput({ direction }),
            onAttack: () => network.sendAttack({ attackId: `atk-${Date.now()}` }),
            onSkill: (skillId) => network.sendCastSkill({ skillId }),
            onPickup: () => {
                const state = runtime.getState();
                const self = state.players.find((p) => p.id === state.selfPlayerId);
                if (!self)
                    return;
                const nearby = state.drops.find((drop) => Phaser.Math.Distance.Between(drop.x, drop.y, self.x, self.y) <= 150);
                if (nearby)
                    network.sendPickup({ dropId: nearby.id });
            },
...
function createInitialExtractState() {
    return {
        isOpen: false,
        isExtracting: false,
        progress: null,
...
function resolveExtractOpen(payload) {
    if (!payload)
        return true;
    return payload.opened ?? payload.available ?? true;
}
function resolveCountdownSeconds(payload) {
    if (typeof payload?.remainingMs === "number")
        return Math.max(0, Math.ceil(payload.remainingMs / 1000));
    return null;
}
function normalizeExtractProgress(payload) {
...
function normalizeInventoryEvent(payload) {
    ...
    return {
        ...
        equipment: Object.fromEntries(Object.entries(rawEquipment).flatMap(([slot, item]) => {
            if (!isRecord(item))
                return [];
            return [[slot, {
                        instanceId: asString(item.instanceId, cryptoId()),
...
function normalizeItemModifiers(value) {
    if (!isRecord(value))
        return undefined;
    return {
...
function normalizeAffixes(value) {
...
function asOptionalStringValue(value) {
