...
export class GameScene extends Phaser.Scene {
    ...
    monsterMarkers = new Map();
    dropMarkers = new Map();
    chestSprites = new Map();
    chestLabels = new Map();
    terrainLayer;
    detailLayer;
    obstacleLayer;
    ...
    preload() {
        ...
        crateCtx.beginPath();
        crateCtx.moveTo(4, 4);
        crateCtx.lineTo(44, 44);
        crateCtx.moveTo(44, 4);
        crateCtx.lineTo(4, 44);
        crateCtx.stroke();
    ...
    handleCombatResult(payload) {
        ...
        if (attackerMonster && targetPlayer) {
            this.showMonsterAttackVfx(attackerMonster.root.x, attackerMonster.root.y, targetPlayer.root.x, targetPlayer.root.y, attackerMonster.root.depth);
        }
    }
    ...
    handleSkill() {
        ...
        if (skillId) {
            ...
            const selfPlayer = this.latestState?.players.find(p => p.id === selfPlayerId);
            if (selfPlayer)
                this.createSkillVfx(selfPlayer.x, selfPlayer.y, 0x38bdf8);
            if (selfPlayerId && skillId === "sword_dashSlash") {
                const marker = this.playerMarkers.get(selfPlayerId);
                if (marker) {
                    for (let i = 0; i < 3; i++)
                        this.time.delayedCall(i * 50, () => marker.createGhost());
                }
            }
        }
    }
    createWeaponVfx(x, y, weaponType, direction) {
        ...
        if (weaponType === "sword") {
            ...
            g.beginPath();
            g.moveTo(Math.cos(angle) * 16 + Math.cos(angle + Math.PI / 2) * 10, Math.sin(angle) * 16 + Math.sin(angle + Math.PI / 2) * 10);
            g.lineTo(Math.cos(angle) * 16 + Math.cos(angle - Math.PI / 2) * 10, Math.sin(angle) * 16 + Math.sin(angle - Math.PI / 2) * 10);
            g.strokePath();
            this.tweens.add({
                targets: g,
                alpha: 0,
    ...
    update(time, delta) {
        ...
        if (this.obstacleLayer) {
            this.obstacleLayer.list.forEach((obj) => {
                if (obj.setDepth)
                    obj.setDepth(obj.y);
            });
        }
    ...
    shutdown() {
        ...
        for (const marker of this.dropMarkers.values())
            marker.destroy();
        for (const label of this.chestLabels.values())
            label.destroy();
        this.chestSprites.clear();
        this.chestLabels.clear();
        this.regionLabels = [];
    }
    shakeCamera(intensity = 0.005, duration = 100) {
        this.cameras.main.shake(duration, intensity);
    }
    flashEffect(target) {
        if (!(target instanceof Phaser.GameObjects.Sprite || target instanceof Phaser.GameObjects.Container))
            return;
    ...
    emitMoveInput(time) {
        ...
        if (!changed && time - this.lastMoveSentAt < 60)
            return;
    ...
    syncPlayers(state) {
        for (const player of state.players) {
            ...
            if (existing)
                existing.sync(player, player.id === state.selfPlayerId);
            else
                this.playerMarkers.set(player.id, new PlayerMarker(this, player, player.id === state.selfPlayerId));
        }
    }
    syncMonsters(state) {
    ...
    showTutorial() {
        ...
        const content = this.add.text(10, 40, moveHint + "\n" +
            actionHint + "\n\n" +
            "目标: 击杀怪物, 收集战利品\n前往中心区域撤离。", {
            fontFamily: "monospace",
            fontSize: "13px",
